...
function docker.true {
    #accepts 0 args.  
    #returns true/false if is a container under docker
    ...
    if [ `${cmd_echo} "${lcgroup_pid1}" | ${cmd_grep} -ci docker` -gt 0 ]; then
        lexitcode=${exitok}
        lexitstring=${true}
    else
        lexitcode=${exitcrit}
    ...
    if [ `${cmd_echo} "${lcgroup_pid1}" | ${cmd_grep} -ci ^1:` -eq 0 ]; then
        lexitcode=${exitok}
        lexitstring=${true}
    else
        lexitcode=${exitcrit}
...
function docker.report {
    #accepts 0 args.  
    #returns json of docker ps and image status as json
    #relies on global variable odo for systems utilizing docker running under odo
